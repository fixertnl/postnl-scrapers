...
**`postnl_start_werktijd` is sowieso altijd maar een fallback** — voor DH/WVN-ritten met een shiftnummer is `shift_tijden` leidend (zie Werkuren hierboven), dus dit veld eindigt alleen daadwerkelijk zichtbaar als begintijd voor RTM/@Home-ritten, reserve-ritten, of een DH/WVN-rit zonder shift_tijden-data voor die datum. Sinds de generalisatie hieronder (2026-09-03, na een vervolgvraag op issue #107) gebruikt de eerste-detectie-toewijzing (`stopsTeDoen < stopsTotaal`, nog geen `postnl_start_werktijd`) daarom voor **alle** ritten dezelfde, nauwkeurigere bron — niet meer alleen voor reserve-ritten:

- **Algemene regel (elk ritsoort, behalve ad-ritten hieronder)** — niet het loutere detectiemoment (`nu`, kan een willekeurige pollingtijd ná de echte eerste bezorging zijn), maar de op dát moment gelezen "Tijdstip laatste actie" zelf (via `nlTijdstipNaarIso()`), met terugval op `nu` als die tekst onparseerbaar is. Was tot 2026-09-03 alleen zo voor reserve-ritten (`isReserveRitnummer`); voor regulier/RTM/@Home werd nog gewoon `nu` gebruikt. Geen reden om dat onderscheid te maken — `nu` is voor geen enkel ritsoort een betere proxy dan de eerste gelezen "laatste actie".
- **Ad-ritten (avond)** — `isAdKanaal(kanaal)`: kanaal-code `AD`. Enige uitzondering: krijgen `shift: null` (geen shiftlabel — er is toch geen shift_tijden-data voor avondritten) en een **vaste begintijd van 17:00** (`shiftTijdNaarIso(datum, '17:00')`) bij eerste detectie, in plaats van "laatste actie" — avondritten beginnen per definitie om 17:00, ongeacht wanneer de eerste stop bezorgd werd.
- **Reserve-ritten** — `isReserveRitnummer(ritnummer)`: een 4-cijferig ritnummer, of het eerste cijfer van het ritnummer is `9`. Volgen sinds de generalisatie de algemene regel hierboven (geen aparte tak meer nodig). Wel nog een aparte uitsluiting in het "verdwenen-uit-grid"-pad: de shiftMap-lookup daar wordt voor reserve-ritten overgeslagen (een 4-cijferig ritnummer kan toevallig met zijn eerste cijfer een échte shift 1-6 raken en zou dan diens begintijd lenen als plausibiliteits-startMs), `postnl_start_werktijd` is daar al de juiste bron.
